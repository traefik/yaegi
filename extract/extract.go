...
func matchList(name string, list []string) bool {
	for _, re := range list {
		if re == "" {
			continue
		}
		if m, e := regexp.MatchString(re, name); e == nil && m {
			return true
		}
	}
	return false
